...
> **Last updated:** 2026-06-24 (D28: studio hardened (graceful rig-failure fallback) + in-browser MESH animation w/ sway/drift/manual-bone controls; splat-deform binding is next)
...
- **NEXT:** **bind splats→rig** so the SPLAT (not just the mesh) animates — plan: server-side weight transfer (nearest mesh-vertex skin weights → each splat, in a shared frame) baked into the splat, then GPU LBS in the viewer (per-point skinIndex/weight + bone-matrix uniforms). Then motion sources (procedural/AnimaX) + ElevenLabs audio. Single-image quality note: one view hallucinates the back (front copied over) — multi-view mitigates; keep for later refinement. Note: ElevenLabs = audio only; movement is a SEPARATE source (procedural/motion model). First MVP demo likely = rigged MESH animated in Three.js + ElevenLabs audio (splat-binding fidelity is a later upgrade). Render fidelity caveat: real.html's isotropic-additive viewer makes splats look sparse — validate true quality via superspl.at or integrate mkkellogg.
