...
- **UniRig setup (in progress):** venv `/home/sov2/projects/unirig-venv` (**Python 3.11.15** via uv — bpy needs 3.11), torch 2.11+cu128, bpy 4.2, spconv 2.3.8, all bulk deps ✅. Clone at `/home/sov2/projects/UniRig`. Crab mesh exported → `generation/out/crab_mesh.obj` (464k verts).
  ...
  - **SKIN weights (next):** `unirig_skin` actually CALLS `flash_attn.modules.mha.MHA` + `torch_scatter.segment_csr`. torch_scatter stub already real. PLAN: replace flash_attn MHA stub with a NATIVE (sdpa-backed) MHA implementation matching UniRig's call signature → avoids the brutal flash_attn compile. Fallback = real flash_attn build (long, chunked, 10-min-cap risk).
