...
> **Last updated:** 2026-06-24 (D26: rig+skin proven end-to-end — human 28 bones / butterfly 42 bones, both 100% skinned, from TRELLIS→cleanup→UniRig; mesh-cleanup script added; viewer now multi-subject)
...
- **NEXT (post-D26):** the rig+skin path is DONE end-to-end for clean subjects (human 28 / butterfly 42 bones, both 100% skinned). Remaining for the demo: **bind splats→rig** (geometric nearest-triangle, no per-asset training) + **drive motion** (procedural or AnimaX) + **ElevenLabs audio**. Skinned GLBs (`assets/{human,butterfly}_rigged.glb`) are already Three.js-animatable. Note: ElevenLabs = audio only; movement is a SEPARATE source (procedural/motion model). First MVP demo likely = rigged MESH animated in Three.js + ElevenLabs audio (splat-binding fidelity is a later upgrade). Render fidelity caveat: real.html's isotropic-additive viewer makes splats look sparse — validate true quality via superspl.at or integrate mkkellogg.
