...
- **UniRig setup (in progress):** venv `/home/sov2/projects/unirig-venv` (**Python 3.11.15** via uv — bpy needs 3.11), torch 2.11+cu128, bpy 4.2, spconv 2.3.8, all bulk deps ✅. Clone at `/home/sov2/projects/UniRig`. Crab mesh exported → `generation/out/crab_mesh.obj` (464k verts).
  - **Skeleton model (unirig_ar) is CLEAN** — no flash_attn/scatter/cluster → should run on current env.
  - **WALL: skin-weight model needs `flash_attn` + `torch_scatter` + `torch_cluster`** — all need CUDA-toolkit (nvcc) source compilation on Blackwell. No prebuilt wheels for torch 2.11. flash_attn build is long (~1-2hr) AND the env caps commands at 10min (144-kill) → must chunk via ninja incremental, OR find wheels. This is the real blocker for full rigging.
