...
> **Last updated:** 2026-06-24 (D29: splat bound to rig — animated SPLATS via GPU point-skinning; viewer fully assembled (splat+mesh+skeleton one frame); bind verified numerically, shader pending in-browser check)
...
- **D31 — Two renderers (the "dots vs beautiful" reality) + STRATEGIC pivot question (OPEN).** Clarified for the user: `benchmark/splat.html` looks beautiful because it uses **mkkellogg/gaussian-splats-3d** (real anisotropic depth-sorted 3DGS). The **studio/viewer use a crude `THREE.Points` (dots)** renderer ON PURPOSE — mkkellogg (a) conflicts (2× three.js instances) and (b) renders only STATIC gaussians (can't deform). So: beautiful STATIC splat = solved (mkkellogg); **beautiful ANIMATED splat = hard** (needs a custom skinned-gaussian rasterizer — research-grade "animatable gaussians"). Current animated splat = dots only. Fixes shipped: splat-mesh alignment via ICP/Umeyama (chamfer 0.045→0.021, scale now exact); studio defaults to splat-first (mesh hidden — splat is the hero/product, mesh is an inspection layer); stronger visible sway. **OPEN STRATEGIC DECISION (user leaning toward pivot):** focus on the curated **3D library + retrieval** (the §4 moat) and treat generation as the curated-asset producer + a quality-GATED user miss-path, rather than perfecting live user-generation/animation now. Recommendation = PIVOT: generation already works well enough for us to curate; bad user-gen is handled by the write-time ROUTER + a quality scorer (geometry/rig sanity, multi-view coverage) → reject/flag/fallback-to-retrieval, which is LIBRARY work. Animated-beautiful-splats + live user-gen are deferred. Awaiting user confirmation.
