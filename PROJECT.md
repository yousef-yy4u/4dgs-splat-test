...
**Benchmarking fairness:** splat render is FILL-RATE-bound → render resolution dominates cross-device comparison. Benchmark both devices at a FIXED resolution (added `Res: 1280x720` toggle) — fairer AND more realistic, since the phone renders to the GLASSES resolution we choose, not its own retina screen. Cross-browser (Chrome vs Safari) = minor variable; bigger confounds = resolution/DPR + phone power/thermal throttle. (Note: modern Ryzen iGPU can genuinely beat a thermally-limited phone — trust the data over the earlier A15>iGPU assumption.)
