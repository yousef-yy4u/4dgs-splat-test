...
> **Last updated:** 2026-06-24 (D25: 1-bone skeleton root-caused — rigging stack verified good (giraffe=44 bones); crab mesh is the OOD/noisy input; viewer bones fixed)
...
- **UniRig setup (in progress):** venv `/home/sov2/projects/unirig-venv` (**Python 3.11.15** via uv — bpy needs 3.11), torch 2.11+cu128, bpy 4.2, spconv 2.3.8, all bulk deps ✅. Clone at `/home/sov2/projects/UniRig`. Crab mesh exported → `generation/out/crab_mesh.obj` (464k verts).
  ...
  - **VIEWER FIX (`benchmark/viewer.html`):** bones "didn't render / not a single dot" because `THREE.SkeletonHelper` only draws bone→child line SEGMENTS — a single childless/zero-length bone = zero segments = nothing visible. Added **joint spheres at every bone** (always visible, even for 1 degenerate joint) + a **"Giraffe rig (known-good)" toggle** loading `assets/giraffe_rigged.glb` (44 bones) so the viewer's multi-bone rendering is provable. Info line now shows live crab vs giraffe bone counts. Asset built via `generation/prep_viewer.py <fbx> <glb>`.
...
- **NEXT (post-D25):** generate a **rig-friendly** subject (clear biped/quadruped/character) + clean the TRELLIS mesh (largest component, strip internal shells) → re-run skeleton → expect a real multi-bone armature → skin → bind splats → animate. The rigging stack itself is verified working (giraffe = 44 bones). Note: ElevenLabs = audio only; movement is a SEPARATE source (procedural/motion model). First MVP demo likely = rigged MESH animated in Three.js + ElevenLabs audio (splat-binding fidelity is a later upgrade). Render fidelity caveat: real.html's isotropic-additive viewer makes splats look sparse — validate true quality via superspl.at or integrate mkkellogg.
